...
1. You pick a skill, for example `feature-bug-analyst`.
...
2-3 cases per skill in `skills/<family>/<id>/evals/evals.json`. Each case has a prompt, a tiny fake app (fixture), optional scripted answers, and a list of checks (file exists, Status is `ready-for-developer`, no source edits, and so on).
...
2. Copy extra files on top if needed (for example a planted bug under the skill's `evals/files/`). Overlay paths resolve in the skill folder first, then at the pack root.
...
./run-quality-eval.ps1 -Skill feature-bug-analyst -BaselineCommit HEAD~1
```

`-Skill all` runs every skill. `-EvalName empty-cart-exact-report` runs one case. The runner does not install a CI job.
...
Start with 2-3 prompts per skill. Vary phrasing. Include one edge (refuse the wrong sibling skill, missing evidence, no test runner). Checks should be things a script can fail: file exists, Status/Verdict enum, template headings, no emoji, no `concept.md`, git branch rules, source untouched for analysis-only skills.
