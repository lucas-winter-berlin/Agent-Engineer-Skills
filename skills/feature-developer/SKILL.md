...
description: >-
  Implements a locked what-to-build file in the current repo: feature branch,
...
The user wants the locked file built in this repo.
...
3. **Implement.** Make the in-scope behavior true. Obey walls and locks. Match local style. If the project already has a test command, run it and add tests only in the existing pattern. If there is no test runner, do not invent one.
...
## what-to-build.md is law

- Follow in scope, locks, done-when, do-not.
- If that file is missing or still ambiguous, stop and run `feature-specifier`. Do not guess product.
- If two real technical options remain, stop and ask. Do not pick a library or store the lock did not name.
- Do not "improve" the product unless the file locked it.
- If implementation discovers a landmine the file did not lock, stop and ask.
