...
Input: `agent-engineer-skills/<feature-name>/what-to-build.md` from `feature-specifier`. If that folder is missing but `docs/features/<feature-name>/` exists, use the old folder. If only an older `concept.md` exists, treat it as `what-to-build.md`.
...
Do not use when the idea is still fuzzy (`feature-specifier`), when they want a review of shipped code (`feature-code-reviewer`), or only tests (`feature-tester`).
...
4. **Describe.** Write `agent-engineer-skills/<feature-name>/what-was-implemented.md` from [templates/what-was-implemented.md](templates/what-was-implemented.md): what you did, why, where, how to try it. Full context for a reviewer who did not see the chat. Not a second spec. If the spec still lives under `docs/features/<feature-name>/`, write next to it.
...
Code is on the feature branch. The write-up is `what-was-implemented.md`. Stop. Do not start `feature-code-reviewer` or `feature-tester` unless the operator named that skill.
